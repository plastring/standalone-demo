...
execRunner () {
  ...
  exec "$@" > /dev/null 2>&1 &
  echo $! > $app_home/../bin/Standalone-App.pid
}
addJava () {
  dlog "[addJava] arg = '$1'"
  java_args+=( "$1" )
...
declare -r app_mainclass="com.standalone.demo.App"

for i in $lib_dir/*.jar; do
  libs=$libs:$i
done

# declare -r app_classpath="$libs:$app_home/../conf"
declare -r app_classpath="$app_home/../conf:$libs"
